...
export async function downloadFactuurPdf(factuur: Factuur) {
  ...
  doc.text(`Datum: ${formatDatum(factuur.created_at)}`, pageWidth - margin, 22, { align: "right" });
  ...
  if (installatiegegevens) {
    ...
    if (installatiegegevens.datum) {
      doc.text(`Installatiedatum: ${formatDatum(installatiegegevens.datum)}`, margin, y);
      y += 6;
    }
  ...
  doc.text(`Bedrag excl. btw: ${formatBedrag(factuur.bedrag)}`, margin, y);
  y += 6;
  doc.text(`Btw (21%): ${formatBedrag(factuur.btw)}`, margin, y);
  y += 6;
  divider();
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(13);
  doc.text(`Totaal te betalen: ${formatBedrag(factuur.totaal)}`, pageWidth / 2, y + 10.5, { align: "center" });
  y += 26;
  ...
  if (factuur.betaaldatum) {
    doc.text(`Betaald op: ${formatDatum(factuur.betaaldatum)}`, margin, y);
    y += 6;
  }
